...
class TestTimeEntryImport(OpenProjectBackendTestCase):
    ...
    def test_time_entry_is_created_7_3(self):
        self.backend.version = '7.3'
        with get_openproject_mocker(base_version='7.3'):
    ...
    def test_time_entry_is_created_9_0(self):
        with get_openproject_mocker(base_version='9.0'):
            self.backend.import_projects(delay=False)
            self.backend.op_project_ids.filtered(
                lambda p: not p.openproject_id == '6').write(
                    {'op_sync': False})
